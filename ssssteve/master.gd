...
func _ready():
	...
	triangle.set_name("tri1")
	triangle.mesh = PrismMesh.new()
	self.add_child(triangle)
...
func _physics_process(delta): 
	...
	triangle.global_position = new_point_rot # poog that rotation is right
	
	# test for all other points 
	for x in range(0, curve_array_point.size()):
		curve_array_point[x] = $Player/Path3D.curve.get_point_position(x)
	for x in range(0, curve_array_in.size()):
		curve_array_in[x] = $Player/Path3D.curve.get_point_in(x)
	for x in range(0, curve_array_out.size()):
		curve_array_out[x] = $Player/Path3D.curve.get_point_out(x)
	print("array of points in curve ", curve_array_point)
	
	
	# rotate players Path3 to face snake  
	$Player/Path3D.look_at($Path3D/PathFollow3D18/MeshInstance3D.global_position,Vector3(0,1,0)) # need to get the real position of the cube , 
	...
	if path_handle_18.get_progress() > line.curve.get_baked_length():
		...
		while (i < 15) and not ensnared:
			...
			var curve_point_out = move_point(curve_array_point[i],$Player/Path3D)
...
func move_point(point:Vector3,object:Node3D):
	...
	#print("first position on player , should change , ",$Player/Path3D.curve.get_point_position(0) + translation)
